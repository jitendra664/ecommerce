...

{
    ...
    public function wholesalerange()

    {

        return view('frontend.partials.wholesale_products_section');
    ...
    public function handlewebhooks(Request $request){
        ...
        //  $apiKey = '94563b294f14f94dedd4fcb0bee598b5';
        // $formID = '232299327708869';
        // $response = Http::get("https://api.jotform.com/form/{$formID}/submissions?apiKey={$apiKey}");
        // $data = $response->json();
        // $inputdata = json_encode($data);
        // return response()->json(['message' => 'JotForm data received',$inputdata]);
      
        // $data = json_encode($_POST, true);
        //  file_put_contents('resources/views/webhook_test.blade.php', print_r($data, true), FILE_APPEND);
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                $csvFile = 'jotform_data.csv';
            
            
                $csvData = implode(',', array_keys($jotformData)) . "\n";  
                $csvData .= implode(',', $jotformData) . "\n"; 
            
               $csvFilePath = storage_path('jotform_data.csv');
              $csv = Writer::createFromPath($csvData, 'w+');
              $csv->insert([$csvData]);
              $data = [
    $csvData];
    $csv->insertAll($data);
    $csv->output();
                ...
                // file_get_contents("jotform_data.csv", $csvData, FILE_APPEND);
            
                echo "JotForm data saved to CSV file.";
            } else {
                echo "data received.";
            }
